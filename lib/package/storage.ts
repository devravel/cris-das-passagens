import "server-only";

import { isValidBlogImageUrl, normalizeBlogImageUrl } from "@/lib/blog/image-url";
import { compressImageForStorage } from "@/lib/storage/compress-image";
import { STORAGE_UPLOAD_CACHE_CONTROL } from "@/lib/storage/media-url";
import { createSupabaseAdminClient } from "@/lib/supabase-admin";

export const PACKAGE_IMAGES_BUCKET = "package-images";
export const PACKAGE_IMAGES_FOLDER = "package-images";

export const ALLOWED_PACKAGE_IMAGE_MIME_TYPES = new Set([
  "image/jpeg",
  "image/jpg",
  "image/png",
  "image/webp",
  "image/avif",
]);

export const MAX_PACKAGE_IMAGE_BYTES = 5 * 1024 * 1024;

const MIME_TO_EXTENSION: Record<string, string> = {
  "image/jpeg": "jpg",
  "image/jpg": "jpg",
  "image/png": "png",
  "image/webp": "webp",
  "image/avif": "avif",
};

function getSafeExtension(fileName: string, mimeType: string) {
  const fromName = fileName.split(".").pop()?.toLowerCase()?.replace(/[^a-z0-9]/g, "") ?? "";

  if (fromName && fromName.length <= 5) {
    return fromName;
  }

  return MIME_TO_EXTENSION[mimeType.toLowerCase()] ?? "jpg";
}

export function makePackageStoragePath(fileName: string, mimeType: string) {
  const safeExtension = getSafeExtension(fileName, mimeType);
  const randomPart = crypto.randomUUID();

  return `${PACKAGE_IMAGES_FOLDER}/${Date.now()}-${randomPart}.${safeExtension}`;
}

export async function uploadPackageImageToStorage(
  file: File,
): Promise<{ publicUrl: string }> {
  const mimeType = (file.type || "image/jpeg").toLowerCase();

  if (!ALLOWED_PACKAGE_IMAGE_MIME_TYPES.has(mimeType)) {
    throw new Error("Formato inválido. Use JPG, PNG, WEBP ou AVIF.");
  }

  if (file.size > MAX_PACKAGE_IMAGE_BYTES) {
    throw new Error("A imagem deve ter no máximo 5MB.");
  }

  const { buffer, contentType, extension } = await compressImageForStorage(file);
  const path = makePackageStoragePath(`image.${extension}`, contentType);
  const supabaseAdmin = createSupabaseAdminClient();

  const { error: uploadError } = await supabaseAdmin.storage
    .from(PACKAGE_IMAGES_BUCKET)
    .upload(path, buffer, {
      contentType,
      upsert: false,
      cacheControl: STORAGE_UPLOAD_CACHE_CONTROL,
    });

  if (uploadError) {
    throw new Error(`Falha no upload: ${uploadError.message}`);
  }

  const { data } = supabaseAdmin.storage.from(PACKAGE_IMAGES_BUCKET).getPublicUrl(path);
  const publicUrl = normalizeBlogImageUrl(data.publicUrl);

  if (!isValidBlogImageUrl(publicUrl)) {
    throw new Error("Não foi possível gerar a URL pública da imagem.");
  }

  return { publicUrl };
}
