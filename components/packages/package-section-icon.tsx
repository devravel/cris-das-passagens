import { BedDouble, Luggage, Plane } from "lucide-react";

import { cn } from "@/lib/utils";

type PackageSectionIconProps = {
  variant: "suitcase" | "plane" | "bed";
  className?: string;
};

export function PackageSectionIcon({ variant, className }: PackageSectionIconProps) {
  const Icon = variant === "suitcase" ? Luggage : variant === "plane" ? Plane : BedDouble;
    ...
    <div className={cn("relative inline-flex size-16 shrink-0 sm:size-[4.5rem]", className)}>
      ...
      <span
        ...
        className={cn(
          ...
          variant === "plane" && "bottom-0 left-2 size-8 sm:size-9",
        )}
      />
